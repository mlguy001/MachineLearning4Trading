...
-- Create model
CREATE OR REPLACE MODEL `ai4f.directionClassifierBosstedTree`
OPTIONS
  --(model_type='BOOSTED_TREE_CLASSIFIER', input_label_cols=['direction']) AS
...
-- Evaluate the model
SELECT * FROM ML.EVALUATE(MODEL ai4f.directionClassifierBosstedTree,
(
SELECT
  symbol,
  close_MIN_prior_5_days,
...
-- Predict
SELECT * FROM ML.PREDICT(MODEL ai4f.directionClassifierBosstedTree,
(
SELECT
  Date,
  symbol,
